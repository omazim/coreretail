...
var W3 = (function () {
    ...
    function w3IncludeHTMLLocal(service) {
      var z, i, a, file, path, snippet;
      z = document.getElementsByTagName("*");
      for (i = 0; i < z.length; i++) {
        if (z[i].getAttribute("w3-include-html-local")) {
          a = z[i].cloneNode(false);
          file = z[i].getAttribute("w3-include-html-local");
          path = Service.getHTMLSnippetFullPath(file);
          snippet = "";//rethink: Service.getFileContent(path);
            if (snippet.length > 0) {
              a.removeAttribute("w3-include-html-local");
              a.innerHTML = snippet;
              z[i].parentNode.replaceChild(a, z[i]);
              w3IncludeHTML();
            }
    ...
    return {
        display: w3DisplayData,
        include: w3IncludeHTML,
        includeLocal: w3IncludeHTMLLocal,
        http: w3Http
    };
})();
